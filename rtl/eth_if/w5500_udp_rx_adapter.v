...
module w5500_udp_rx_adapter #(
    ...
    localparam [15:0] LOCAL_PORT        = 16'd80;
    ...
    function [7:0] synth_header_byte;
        ...
        begin
            ...
            case (idx)
                ...
                16'd36: synth_header_byte = LOCAL_PORT[15:8];
                16'd37: synth_header_byte = LOCAL_PORT[7:0];
        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                ST_SOCKET_CFG: begin
                    if (!seq_active && !seq_done) begin
                        case (state_step)
                            5'd0: start_spi_write(S0_RXBUF_SIZE, CTRL_S0_REG_WRITE, SOCKET_BUF_16KB);
                            5'd1: start_spi_write(S0_TXBUF_SIZE, CTRL_S0_REG_WRITE, SOCKET_BUF_16KB);
                            5'd2: start_spi_write(S0_MR, CTRL_S0_REG_WRITE, S0_MR_UDP);
                            5'd3: start_spi_write(S0_PORT0, CTRL_S0_REG_WRITE, LOCAL_PORT[15:8]);
                            5'd4: start_spi_write(S0_PORT1, CTRL_S0_REG_WRITE, LOCAL_PORT[7:0]);
                            default: begin
                                state      <= ST_WAIT_LINK;
                                state_step <= 5'd0;
                            end
                ...
                ST_SOCKET_OPEN: begin
                    if (!seq_active && !seq_done) begin
                        ...
                        if (state_step == 5'd0) begin
                            state_step <= 5'd1;
                        end else if (seq_rx[3] == S0_STATUS_UDP) begin
                            init_busy       <= 1'b0;
                            init_done       <= 1'b1;
                            rx_poll_wait_ctr<= 16'd0;
                            state           <= ST_READ_PHY;
                ...
                ST_READ_PHY: begin
                    if (!seq_active && !seq_done) begin
                        case (state_step)
                            ...
                            5'd1:  start_spi_read(S0_MR, CTRL_S0_REG_READ);
                            5'd2:  start_spi_read(S0_SR, CTRL_S0_REG_READ);
                            5'd3:  start_spi_read(COMMON_SHAR0, CTRL_COMMON_READ);
                            5'd4:  start_spi_read(COMMON_SHAR1, CTRL_COMMON_READ);
                            5'd5:  start_spi_read(COMMON_SHAR2, CTRL_COMMON_READ);
                            5'd6:  start_spi_read(COMMON_SHAR3, CTRL_COMMON_READ);
                            5'd7:  start_spi_read(COMMON_SHAR4, CTRL_COMMON_READ);
                            5'd8:  start_spi_read(COMMON_SHAR5, CTRL_COMMON_READ);
                            5'd9:  start_spi_read(COMMON_SIPR0, CTRL_COMMON_READ);
                            5'd10: start_spi_read(COMMON_SIPR1, CTRL_COMMON_READ);
                            5'd11: start_spi_read(COMMON_SIPR2, CTRL_COMMON_READ);
                            5'd12: start_spi_read(COMMON_SIPR3, CTRL_COMMON_READ);
                            default: begin
                                rx_poll_wait_ctr <= 16'd0;
                                state            <= ST_RX_POLL;
                                state_step       <= 5'd0;
                        ...
                        case (state_step)
                            ...
                            5'd3:  shar_value[47:40] <= seq_rx[3];
                            5'd4:  shar_value[39:32] <= seq_rx[3];
                            5'd5:  shar_value[31:24] <= seq_rx[3];
                            5'd6:  shar_value[23:16] <= seq_rx[3];
                            5'd7:  shar_value[15:8]  <= seq_rx[3];
                            5'd8:  shar_value[7:0]   <= seq_rx[3];
                            5'd9:  sipr_value[31:24] <= seq_rx[3];
                            5'd10: sipr_value[23:16] <= seq_rx[3];
                            5'd11: sipr_value[15:8]  <= seq_rx[3];
                            5'd12: sipr_value[7:0]   <= seq_rx[3];
                        endcase

                        if (state_step == 5'd12) begin
                            phy_read_count   <= phy_read_count + 32'd1;
                            rx_poll_wait_ctr <= 16'd0;
                            state            <= ST_RX_POLL;
                            state_step       <= 5'd0;
                ...
                ST_READ_UDP_HDR: begin
                    if (!seq_active && !seq_done) begin
                        start_spi_read(rx_read_ptr + {11'd0, state_step}, CTRL_S0_RXBUF_READ);
                    end else if (seq_done) begin
                        case (state_step)
                ...
                ST_STREAM_FRAME: begin
                    if (frame_index < SYNTH_HDR_BYTES) begin
                        if (frame_ready) begin
                            frame_valid    <= 1'b1;
                            frame_data     <= synth_header_byte(frame_index, udp_payload_len, peer_ip, peer_port);
                        ...
                        start_spi_read(rx_read_ptr + UDP_RECORD_BYTES + (frame_index - SYNTH_HDR_BYTES), CTRL_S0_RXBUF_READ);
                    end else if (seq_done) begin
                        frame_valid    <= 1'b1;
                        frame_data     <= seq_rx[3];
                        frame_sop      <= 1'b0;
                ...
                ST_COMMIT_RX: begin
                    if (!seq_active && !seq_done) begin
                        case (state_step)
                            5'd0: start_spi_write(S0_RX_RD_MSB, CTRL_S0_REG_WRITE, next_rx_read_ptr[15:8]);
                            5'd1: start_spi_write(S0_RX_RD_LSB, CTRL_S0_REG_WRITE, next_rx_read_ptr[7:0]);
                            5'd2: start_spi_write(S0_CR, CTRL_S0_REG_WRITE, S0_CR_RECV);
                            default: begin
                                rx_packet_seen   <= 1'b1;
                                rx_poll_wait_ctr <= 16'd0;
                                state            <= ST_RX_POLL;
